...
** Last update Tue Feb  7 18:15:04 2017 Gregoire Renard
*/
...
static void		*build_page(t_alloc **middle,
				    t_alloc **ex_end,
				    int new_size)
{
  void			*start;
				  ...
				  int size)
{
  ...
  if (size > (int)(gps - (2 * sizeof(t_alloc) + 1)))
    {
      if (build_page(this, &tmp, size + 2 * sizeof(t_alloc)) == NULL)
	return (NULL);
    }
  else
    if (build_page(this, &tmp, getpagesize()) == NULL)
      return (NULL);
}
